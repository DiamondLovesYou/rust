...
impl Process {
    ...
    #[cfg(all(not(stage0), not(target_os = "nacl")))]
    pub fn spawn<K, V, C, P>(cfg: &C, in_fd: Option<P>,
                              out_fd: Option<P>, err_fd: Option<P>)
                              -> IoResult<Process>
        where C: ProcessConfig<K, V>, P: AsInner<FileDesc>,
    ...
    #[cfg(all(target_os = "nacl", not(stage0)))]
    pub fn spawn<K, V, C, P>(_cfg: &C, _in_fd: Option<P>,
    ...
    #[cfg(target_os = "nacl")]
    pub fn wait(&self, _deadline: u64) -> IoResult<ProcessExit> {
        Err(permission_denied())
    }
