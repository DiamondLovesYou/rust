...
to build a cross compiler for your desired target yourself: the binary distributions
...
Rust, where `NACL_SDK_ROOT` points to where `pepper_canary` is installed, with:
    ...
    $ /path/to/rust/configure --target=le32-unknown-nacl --nacl-sdk-root=$NACL_SDK_ROOT
...
To use with Cargo, run Cargo with:
`CC_le32_unknown_nacl=$($NACL_SDK_ROOT/tools/nacl_config.py -t pnacl --tool=cc)
CXX_le32_unknown_nacl=$($NACL_SDK_ROOT/tools/nacl_config.py -t pnacl --tool=cxx)
AR_le32_unknown_nacl=$($NACL_SDK_ROOT/tools/nacl_config.py -t pnacl --tool=ar)
cargo build --target=le32-unknown-nacl`

Once you have a `pexe` from Cargo, you can do one of three things:

1. Produce a stable `pexe` for use as a PNaCl module;
2. Translate the bitcode into a Native Client module (ie for use in the Chrome
   Store); or
...
* [`rust-ppapi`][rust-ppapi]: Rust bindings to the (some parts only currently;
  patches welcome!) C Pepper API.
